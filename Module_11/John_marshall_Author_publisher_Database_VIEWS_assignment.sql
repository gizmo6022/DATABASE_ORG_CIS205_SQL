...
CREATE VIEW whoWrote AS
    ...
    FROM
        Author
    JOIN Books
    WHERE Author.aEmail = Books.aEmail_FK;

    SELECT * From whoWrote;


--author and how much they made from a book contract
CREATE VIEW bookEarnings AS
    SELECT 
        Author.aName, 
        Books.title,
        Contracts.dollarSum
    FROM
        Author
    LEFT JOIN Books
    ON Author.aEmail = Books.aEmail_FK
    JOIN Contracts
    ON Books.pEmail_FK = Contracts.pEmail_FK;

    SELECT * From bookEarnings;
